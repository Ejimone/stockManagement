...
const MetricCard: React.FC<{ label: string; value: string | number; context?: string; icon?: React.ReactNode }> = ({ label, value, context, icon }) => (
  <View style={styles.card}>
    ...
    {icon || <View style={styles.cardIconPlaceholder}><Text>Icon</Text></View>}
  </View>
);
...
export default function AdminDashboardScreen() {
  ...
  if (error && !stats) { // Show error only if no stats are available (e.g. initial load failed)
    return (
  ...
  return (
    ...
    >
      ...
      {error && <Text style={[styles.errorText, {marginBottom: 10}]}>Error: {error} (Retrying might show cached data or new data)</Text>}
      ...
      {stats ? (
        <>
          ...
          {stats.total_products !== undefined && (
            <MetricCard label="Total Products" value={stats.total_products.toString()} />
          )}
          {stats.out_of_stock_products !== undefined && (
            <MetricCard label="Out of Stock Products" value={stats.out_of_stock_products.toString()} />
          )}
          {stats.pending_payments !== undefined && (
             <MetricCard label="Pending Payments Value" value={formatCurrency(stats.pending_payments)} context="Total from unpaid sales" />
          )}
        </>
      ) : (
        !isLoading && <Text style={styles.noDataText}>No dashboard data available at the moment.</Text>
      )}
       <View style={styles.buttonContainer}>
        <Button title="Logout (Test)" onPress={async () => {
...
// Re-importing Button because it might have been shadowed if signOut was destructured from useAuth at the top
...
const styles = StyleSheet.create({
  ...
  scrollView: { // This style is not directly used on ScrollView tag, but good to keep if refactoring
  ...
  buttonContainer: { // For the test logout button
    marginHorizontal: 16,
    marginTop: 20,
    marginBottom: 30, // Ensure it's visible if content is short
  },
  buttonSpacing: { // For Retry button
      marginTop: 10,
  }
});
